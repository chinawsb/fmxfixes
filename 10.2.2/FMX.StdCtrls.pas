...
begin
  if FITextSettings <> nil then
    FITextSettings.TextSettings.OnChanged := nil;
...
begin
{+++>}if not FStaysPressed then // 加入此行代码：修正鼠标移开按钮后 IsPressed 会变成 False, Fixed by Aone 2017.04.07
  FIsPressed := False;
  ApplyTriggers;
end;
...
// These variables are introduced only for fixing problem in update, because we cannot change interfaces.
var
  IsPanOnCheckBox: Boolean;
  [Weak] CapturedCheckBox: TCheckBox;
...
begin
  ...
  begin
    ...
    FPressing := True;
    FIsPressed := True;
    CapturedCheckBox := Self;
    IsPanOnCheckBox := False;
    StartTriggerAnimation(Self, 'IsPressed');
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      // Avoiding a changing "Checked" state if user moves cursor or finger through the control.
